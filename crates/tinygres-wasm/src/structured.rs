    ...
    match error_payload(error) {
        ...
        Err(_) => error_payload(
            &EngineError::new(
                ...
                "TinyGres could not encode a structured bridge error",
            )
...
fn terminal_error() -> JsValue {
    JsValue::from_str("TinyGres could not encode a structured bridge error")
}
...
fn serialization() -> EngineError {
    EngineError::new(
        "BRIDGE_SERIALIZATION_ERROR",
        "TinyGres could not encode the structured bridge result",
    )
}
