...
<section><h2 id="write-with-parameters">Write with parameters</h2><p><a href="https://tinyjoin.org/api/the-essentials/using-a-database/query/"><code>query</code></a>() runs one read or write statement, with application values in the <code>$n</code> array so that they never reach the SQL text. The sql tagged template is the same parameterized call in a shorter form, and takes values only, so there is no way to interpolate raw SQL by accident.</p></section>
