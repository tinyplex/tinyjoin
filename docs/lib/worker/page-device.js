...
export class OpfsPageDevice {
    ...
    readPage(pageIdLow, pageIdHigh, target) {
        this.#assertOpen();
    ...
    writePage(pageIdLow, pageIdHigh, source) {
        this.#assertOpen();
        ...
        if (pageId > count) {
            throw new RangeError(`Page ${pageId} cannot be written before page ${count} is allocated`);
        }
        try {
            if (pageId === count) {
    ...
    flush() {
        ...
        try {
            ...
            throw pageStorageError(error, 'STORAGE_WRITE_FAILED', 'TinyJoin could not flush database pages');
        }
    }
    close() {
        if (this.#closed) {
        ...
        try {
            ...
            throw pageStorageError(error, 'STORAGE_CLOSE_FAILED', 'TinyJoin could not close its page device');
        }
    }
    #assertOpen() {
        if (this.#closed) {
            throw new StorageError('STORAGE_CLOSED', 'The TinyJoin page device is closed');
        }
    }
    #readPageCount() {
        let size;
        try {
            ...
            throw pageStorageError(error, 'STORAGE_READ_FAILED', 'TinyJoin could not read the database page count');
        }
        if (!Number.isSafeInteger(size) || size < 0) {
            throw new StorageError('STORAGE_CORRUPT', 'The TinyJoin page file has an invalid byte length');
        }
        if (size > MAX_DATABASE_BYTES) {
            throw new StorageError('STORAGE_DATABASE_TOO_LARGE', `The TinyJoin page file exceeds ${MAX_DATABASE_BYTES} bytes`);
        }
        if (size % PAGE_SIZE !== 0) {
            throw new StorageError('STORAGE_CORRUPT', 'The TinyJoin page file is not aligned to its page size');
        }
    ...
    #repairTornTail() {
        ...
        try {
            ...
            const mapped = pageStorageError(error, 'STORAGE_WRITE_FAILED', 'TinyJoin could not repair a torn trailing database page');
            throw new StorageError('STORAGE_COMMIT_OUTCOME_UNKNOWN', mapped.message);
        }
    }
    #rollbackAppend(originalPageCount) {
        try {
            ...
            const detail = rollbackError instanceof Error && rollbackError.message
                ...
                : '';
            throw new StorageError('STORAGE_COMMIT_OUTCOME_UNKNOWN', `TinyJoin could not roll back a failed page append${detail}`);
        }
    }
}
/** A dependency-free page device for tests and non-persistent runtimes. */
export class MemoryPageDevice {
    #pages = [];
    #closed = false;
    pageCount() {
        this.#assertOpen();
    ...
    writePage(pageIdLow, pageIdHigh, source) {
        ...
        if (pageId > this.#pages.length) {
            throw new RangeError(`Page ${pageId} cannot be written before page ${this.#pages.length} is allocated`);
        }
...
function pageIdFromWords(low, high) {
...
function pageIdForWriteFromWords(low, high) {
    if (!isUint32(low) || !isUint32(high)) {
        throw new RangeError('Page id words must be unsigned 32-bit integers');
    }
    if (high !== 0 || low > MAX_PAGES) {
    ...
    if (low === MAX_PAGES) {
        throw new StorageError('STORAGE_DATABASE_TOO_LARGE', `The TinyJoin database cannot exceed ${MAX_DATABASE_BYTES} bytes`);
    }
    return low;
}
...
function assertExactPage(bytes, label) {
...
function pageOffset(pageId) {
    return pageId * PAGE_SIZE;
}
function readExactly(handle, at, target) {
    let offset = 0;
    try {
        while (offset < target.byteLength) {
            ...
            if (!Number.isInteger(read) ||
                ...
                throw new StorageError('STORAGE_READ_FAILED', 'TinyJoin received a short database page read with no progress', true);
            }
            offset += read;
        }
    }
    catch (error) {
        throw pageStorageError(error, 'STORAGE_READ_FAILED', 'TinyJoin could not read a database page');
    }
}
function writeExactly(handle, at, source) {
    let offset = 0;
    try {
        while (offset < source.byteLength) {
            ...
            if (!Number.isInteger(written) ||
                ...
                throw new StorageError('STORAGE_WRITE_FAILED', 'TinyJoin received a short database page write with no progress', true);
            }
            offset += written;
        }
    }
    catch (error) {
        throw pageStorageError(error, 'STORAGE_WRITE_FAILED', 'TinyJoin could not write a database page');
    }
}
/**
 ...
 */
function writeExistingExactly(handle, at, source) {
    let offset = 0;
    while (offset < source.byteLength) {
        let written;
        try {
        ...
        if (!Number.isInteger(written) ||
            written < 0 ||
            written > source.byteLength - offset) {
            throw unknownWriteOutcome(new Error('The page device returned an invalid write length'));
        }
        if (written === 0) {
            if (offset === 0) {
                throw new StorageError('STORAGE_WRITE_FAILED', 'TinyJoin received a short database page write with no progress', true);
            }
            throw unknownWriteOutcome(new Error('A partial database page write stopped making progress'));
        }
        offset += written;
    }
}
function unknownWriteOutcome(error) {
    const mapped = pageStorageError(error, 'STORAGE_WRITE_FAILED', 'TinyJoin could not write a database page');
    return new StorageError('STORAGE_COMMIT_OUTCOME_UNKNOWN', `TinyJoin could not safely complete an in-place page write: ${mapped.message}`);
}
function pageStorageError(error, fallbackCode, fallbackMessage) {
    ...
    if (name === 'QuotaExceededError') {
        return new StorageError('STORAGE_QUOTA_EXCEEDED', 'The browser has no space available for TinyJoin database pages', true);
    }
