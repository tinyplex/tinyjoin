use js_sys::Reflect;
use tinyjoin_core::{EngineError, MAX_PAGE_COUNT, PAGE_SIZE, PageDevice, PageId, Result};
...
impl WasmPageDevice {
    ...
    pub(crate) fn new(device: JsValue) -> Result<Self> {
        ...
        let page_count = device
            .raw_page_count()
            .map_err(|error| js_device_error(error, "TinyJoin could not read the page count"))
    ...
    fn close_inner(&mut self) -> Result<()> {
        ...
        self.device
            .raw_close()
            .map_err(|error| js_device_error(error, "TinyJoin could not close its page device"))
    }
}
...
impl PageDevice for WasmPageDevice {
    ...
    fn read_page(&mut self, id: PageId, destination: &mut [u8]) -> Result<()> {
        ...
        let transferred = self
            ...
            .map_err(|error| js_device_error(error, "TinyJoin could not read a database page"))?;
    ...
    fn flush(&mut self) -> Result<()> {
        self.device
            .raw_flush()
            .map_err(|error| js_device_error(error, "TinyJoin could not flush database pages"))
    }
}
